...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    while ($currentDate <= $endDateObj) {
        ...
        if (in_array($dayOfWeek, $days)) {
            ...
            // Calculate number of slots based on the time difference
            ...
            for ($i = 0; $i < $numSlots; $i++) {
                // Create slots with the correct start time and duration
                $slots[] = [
                    'start_date' => $currentDate->format('Y-m-d'), // Use the current date
                    'start_time' => $dayStart->format('H:i:s'), 
                    'end_date' => $currentDate->format('Y-m-d'), // Same day as start
                    'end_time' => $dayStart->add($duration)->format('H:i:s'), // Add duration to start time
                ...
                // Reset the start time for the next slot
                $dayStart = clone $dayStart;
            }
        }
